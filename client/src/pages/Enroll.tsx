  ...
  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
    ...
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <Input data-testid="input-name" placeholder="Jane Doe" value={data.name} onChange={e => onChange('name', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded focus-visible:ring-airborne-teal" />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Email</label>
        <Input data-testid="input-email" placeholder="jane@example.com" type="email" value={data.email} onChange={e => onChange('email', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded focus-visible:ring-airborne-teal" />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Date of Birth</label>
        <Input data-testid="input-dob" type="date" value={data.dob} onChange={e => onChange('dob', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded focus-visible:ring-airborne-teal" />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Emergency Contact</label>
        <div className="space-y-2">
          <Input data-testid="input-emergency-name" placeholder="Contact Name" value={data.emergencyContactName} onChange={e => onChange('emergencyContactName', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded focus-visible:ring-airborne-teal" />
          <Input data-testid="input-emergency-phone" placeholder="Contact Number" value={data.emergencyContactPhone} onChange={e => onChange('emergencyContactPhone', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded focus-visible:ring-airborne-teal" />
        </div>
      </div>
      <div className="space-y-1">
...
const MembershipSelection = ({ onNext, onBack, onAddPlan, onRemovePlan, selectedPlans, categories, plans }: any) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 -mx-6 px-6 scrollbar-hide">
        {categories.map((cls: ClassCategory) => (
          <button key={cls.id} onClick={() => setActiveTab(cls.name)} data-testid={`button-category-${cls.id}`} className={cn("flex-shrink-0 px-4 py-3 rounded text-sm font-medium border relative", activeTab === cls.name ? "bg-gray-900 text-white border-gray-900" : selectedPlans.some((p: any) => p.category === cls.name) ? "bg-teal-50 text-airborne-teal border-airborne-teal" : "bg-white text-gray-500 border-gray-200")}>
            {cls.name}
            {selectedPlans.some((p: any) => p.category === cls.name) && <span className="absolute -top-1 -right-1 w-3 h-3 bg-airborne-teal rounded-full border border-white" />}
          </button>
        ))}
      </div>
      <div className="space-y-3">
        {currentPlans.map((plan: MembershipPlan) => (
          <div key={plan.id} onClick={() => onAddPlan(activeTab, plan)} data-testid={`card-plan-${plan.id}`} className={cn("p-5 rounded border cursor-pointer", currentPlan?.plan.id === plan.id ? "bg-teal-50 border-airborne-teal" : "bg-white border-gray-100")}>
            <div className="flex justify-between items-center">
  ...
  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
    ...
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <Input data-testid="input-kid-name" placeholder="Kid's Name" value={data.name} onChange={e => onChange('name', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded" />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Kid's Date of Birth</label>
        <Input data-testid="input-kid-dob" type="date" value={data.dob} onChange={e => onChange('dob', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded" />
      </div>
      <div className="space-y-1">
        <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Gender</label>
        <select data-testid="select-kid-gender" value={data.gender} onChange={e => onChange('gender', e.target.value)} className="w-full h-12 px-3 bg-gray-50 border border-gray-100 rounded outline-none focus:ring-1 focus:ring-airborne-teal">
    ...
    <Button onClick={onNext} className="w-full h-12 bg-airborne-teal text-white rounded" data-testid="button-next-kid">Continue</Button>
  ...
  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
    ...
    <div className="space-y-3 pt-2">
      ...
      <div className="space-y-1 pt-2">
        ...
        <Input placeholder="Type Full Name" value={data.signatureName} onChange={e => onChange('signatureName', e.target.value)} className="bg-gray-50 border-gray-100 h-12 rounded focus-visible:ring-airborne-teal" data-testid="input-signature" />
      </div>
    </div>
    ...
    <div className="flex gap-3 mt-6">
      ...
      <Button onClick={onNext} className="flex-1 h-12 bg-airborne-teal hover:bg-airborne-deep text-white rounded shadow-lg shadow-teal-100" data-testid="button-next-waiver">To Payment</Button>
    </div>
  </motion.div>
);
...
export default function Enroll() {
  ...
  const [formData, setFormData] = useState({ name: user?.name || "", email: "", dob: "", emergencyContactName: "", emergencyContactPhone: "", medicalConditions: "" });
    ...
    <MobileLayout>
        <div className="p-6">
            ...
            <AnimatePresence mode="wait">
              ...
              {step === 2 && <MembershipSelection key="step2" categories={categories} plans={plans} selectedPlans={selectedPlans} onAddPlan={handleAddPlan} onRemovePlan={(c: string) => setSelectedPlans(p => p.filter(x => x.category !== c))} onNext={nextStep} onBack={() => setStep(1)} />}
